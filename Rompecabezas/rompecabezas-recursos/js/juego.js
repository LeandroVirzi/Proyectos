...
var instrucciones = ["Utiliza las flechas para jugar","tu tablero debe quedar como el de aqui debajo"];
...
function mostrarInstrucciones(instrucciones) {
  ...
  let id ="lista-instrucciones";
  for(let i = 0; i < instrucciones.length; i++){
    let posI = instrucciones[i];
    mostrarInstruccionEnLista(posI,id);
  };
}
...
function ultimoMovimiento(direccion){
  ...
  actualizarUltimoMovimiento(direccion);//pongo la direccion para que me muestre el ultimo parametro y no todos.
}
...
function chequearSiGano(grilla) {
  for(let i = 0; i < grilla.length; i++){
    for(let j = 0; j < grillaGanadora.length; j++){
...
function intercambiarPosicionesGrilla(filaPos1, columnaPos1, filaPos2, columnaPos2) {
  let posicion1 = grilla[filaPos1][columnaPos1];
  let posicion2 = grilla[filaPos2][columnaPos2];
...
function moverEnDireccion(direccion) {
  ...
  // Mueve pieza hacia arriba, reemplazandola con la blanca
    ...
    nuevaFilaPiezaVacia = filaVacia + 1;//+ estaba con -
    nuevaColumnaPiezaVacia = columnaVacia;
  }
